<!-- src/components/OperatorTicketDetails.vue -->
<template>
      ...
      <v-col cols="12" md="6">
        ...
        <template v-else>
          ...
          />
            ...
            <v-col cols="7" class="value">
              <!-- Za sada lokalno menjamo; ako dodaš PATCH endpoint, ovde triggeruj save -->
              <v-select
...
/>
          ...
          <div class="mt-3">
            <v-btn variant="outlined" class="btn-small" prepend-icon="mdi-paperclip">Open Attachments</v-btn>
          </div>
        </template>
      </v-col>
...
>
 ...
 <v-btn variant="outlined" class="btn-small" @click="escalateDialog = true">
    Escalate
...
/* left column rows */
.label{ font-weight:600; color:rgba(var(--v-theme-on-surface),0.7) }
.value{ white-space:pre-wrap }
.select-compact :deep(.v-field){ height: 34px; }

/* activity grid (vizuelno nalik tabeli iz skice) */
.grid{
  border:1px solid rgba(0,0,0,.25); border-radius:6px; overflow:hidden; font-size:13px;
}
.grid-header, .grid-row{
  display:grid; grid-template-columns: 120px 110px 110px 1fr 110px; align-items:center;
}
.grid-header{ background:rgba(0,0,0,.04); font-weight:600; }
.grid-header > span, .grid-row > span{ padding:6px 8px; border-bottom:1px solid rgba(0,0,0,.12); }
.grid-row:last-child > span{ border-bottom:none; }
.ellipsis{ white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }

/* chat container */
.chat-wrap{ border:1px solid rgba(0,0,0,.4); }
.chat-header{ border-bottom:1px solid rgba(0,0,0,.25); padding:2px 6px; }
.chat-body{ height: 360px; overflow:auto; padding: 12px; background: #fff; }
.msg-row{ display:flex; align-items:flex-end; margin:12px 0; gap:8px; }
.msg-row.right{ justify-content:flex-end; }
.avatar{ opacity:.7; }
.bubble{ max-width: 78%; border-radius: 12px; padding: 10px 12px; box-shadow: 0 1px 4px rgba(0,0,0,.08); white-space: pre-line; }
.bubble.blue{ background: #1976d2; color: #fff; }
.bubble.gray{ background: #e9e9eb; color: #333; }
.timestamp{ font-size: 11px; opacity:.8; margin-top: 6px; text-align: right; }

/* composer i dugmad */
.composer{ display:flex; align-items:center; gap:8px; padding: 8px 12px; border-top:1px solid rgba(0,0,0,.12); }
.composer-input :deep(.v-field){ height: 32px; }
.btn-small{ min-height: 32px; height: 32px; padding: 0 12px; }
.bubble-wrap{
  max-width: 78%;
  display: flex;
...
.footer-row{ display:flex; justify-content:space-between; padding: 8px 12px 12px; }
